...
В данном командном проекте мы реализовали собственную библиотеку string.h на языке программирования Си с некоторыми дополнениями (с собственной реализацией функций sprintf). Библиотека string.h является основной библиотекой языка Си по обработке строк. 
...
В этой части были реализованы следующие функции из библиотеки String.h
...
В этой части была реализована функция sprintf из библиотеки stdio.h. Функция s21_sprintf позволяет форматировать строки с использованием спецификаторов, флагов, ширины, точности и длины.
...
В этой части были реализованы дополнительные модификаторы формата для функции s21_sprintf из библиотеки stdio.h, а именно:
...
В этой части были реализованы некоторые функции обработки строк, вдохновленные классом String в языке C#. Эти функции предоставляют дополнительные возможности для работы со строками.
...
Для всех функий были подготовлены unit-тесты с использованием библиотеки Check для проверки результатов работы функций. Unit-тесты покрывают не менее 80% каждой функции и проверяют результаты работы реализации библиотеки путём сравнения с реализацией стандартной библиотеки string.h.
